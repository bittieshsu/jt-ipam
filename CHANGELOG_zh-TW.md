...
- **順手修掉 Proxmox 的重疊網段隱患。** 它挑子網路用的是 `ORDER BY masklen(cidr) DESC LIMIT 1` —— 兩個單位各有 `192.168.1.0/24` 時會**靜靜挑一個**，可能把 VM 掛到別人的網段底下。改用與其他整合相同的判斷：**歧義就不建**。
...
- **OPNsense / pfSense：「自動建立 IPAM 沒有的位址」開關**（migration 0115，**預設關閉**）。防火牆的 DHCP／ARP 同步原本只會「標記既有的 IP」，對不到就把整筆資料丟掉，而且畫面上完全不說 —— 客戶是自己讀原始碼才查出來的。開啟後，DHCP 租約裡有、IPAM 裡沒有的位址會自動建成紀錄。
  ...
  **落點判斷沿用 LibreNMS 那套**（現抽成 `services/ip_autocreate.py` 三個整合共用一份）：最長首碼比對、**只有唯一命中才建**；重疊網段（兩個單位各有 192.168.1.0/24）造成歧義時**寧可不建**，因為建錯單位比不建更糟。設了「限定子網路範圍」就能消除歧義。
...
- **虛擬化頁面的 IP 可以直接點進 IPAM 的那筆位址。** 資料本來就在系統裡，但看到一台 VM 的位址、想確認它在 IPAM 裡登記成什麼，得自己複製那串數字、切頁、貼上搜尋。
  ...
  **只有剛好對到一筆時才給連結。** 重疊網段下（不同單位共用 `192.168.1.0/24`，那正是本專案存在的理由）同一個位址字串本來就會有多筆紀錄，而無從得知 VM 的位址指的是哪一筆 —— 這種時候維持純文字：給錯的連結比沒有連結更糟，因為使用者會信它。已用 prod 實際資料核對：79 個 VM 網卡位址中，78 個唯一對應而成為連結。
