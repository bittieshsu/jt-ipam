...
### 新增
- **「防火牆規則異動」檢視頁**（異常偵測選單旁，admin 限定）。哨兵通知說「細節到快照裡看」——但 0.5.172 沒有任何畫面能看快照，通知等於指向一個不存在的地方。這一頁列出每筆變更事件的完整 diff（新增／移除／修改，含變了哪些欄位的前後值），初次快照標示為比對基準。規則描述一律以純文字渲染（不進 v-html），注入語句只會是字面文字。

### 修正
- **快照時間改用應用層時間戳。** 原本靠 PostgreSQL 的 `now()`，而它是**交易**時間戳——同一交易內兩份快照時間相同，「取最新一份」的排序會抖動，哨兵可能拿舊快照當基準做 diff。測試在單一交易連拍兩份時抓到。

## [0.5.172] — 2026-08-15

### 新增（資安 × AI 三件套）
- **防火牆規則異動哨兵。** 我們同步三家防火牆（OPNsense／pfSense／FortiGate）的規則，但一直只是「存起來給人看」——沒有任何東西在看它們。半夜多出一條放行規則，是防火牆被入侵或內部人員留後門的經典徵兆，而現況是每輪同步直接覆寫、無聲無息。現在每輪同步把規則正規化後做雜湊，**跟上一份不同才存一列快照並通知管理員**（含逐條 diff；通知矩陣可關）；規則在 UI 被拖動順序**不算**變更（誤報幾次就沒有人會再看告警）、剛接上整合的第一份是 baseline 不告警、規則描述是不可信文字，通知本文由純資料組字，不經 LLM。哨兵自己出錯不會弄壞同步本體。
