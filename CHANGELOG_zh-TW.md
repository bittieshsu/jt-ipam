...
### 新增（資安 × AI 三件套）
- **防火牆規則異動哨兵。** 我們同步三家防火牆（OPNsense／pfSense／FortiGate）的規則，但一直只是「存起來給人看」——沒有任何東西在看它們。半夜多出一條放行規則，是防火牆被入侵或內部人員留後門的經典徵兆，而現況是每輪同步直接覆寫、無聲無息。現在每輪同步把規則正規化後做雜湊，**跟上一份不同才存一列快照（含逐條 diff）**並通知管理員（通知矩陣可關）；規則在 UI 被拖動順序**不算**變更（誤報幾次就沒有人會再看告警）、剛接上整合的第一份是 baseline 不告警、規則描述是不可信文字，通知本文由純資料組字，不經 LLM。哨兵自己出錯不會弄壞同步本體。
