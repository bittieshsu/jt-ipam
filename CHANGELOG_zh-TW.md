...
- **欄位全部拆開**（實際使用回饋）：原本「192.168.1.134:4443（meet）」擠在一欄、「NAT｜名稱」擠在一欄——IP、埠、主機名稱、類型（NAT／規則）、名稱、防火牆（廠牌＋實例）現在各自獨立，可排序比對；NAT 列補上防火牆實例名稱（原本只有廠牌）。
- 加上**欄位選擇**（全站同一套欄位偏好，跨裝置同步）與**防火牆下拉篩選**（清單由資料動態產生——有 manual 或未來新增廠牌不必改程式）。

## [0.5.179] — 2026-08-15

### 新增
- **「對外開放服務」盤點頁**（防火牆規則異動旁；admin 與具全域讀取權的唯讀帳號——稽核員正是這頁的受眾）。彙整「從外面可達的 IP:port」：生效中的 NAT 埠轉發＋WAN 介面上目的為單一 IP 的放行規則，每一項配 IPAM 身分（主機名稱、單位／網段、有無 Wazuh 代理、實際狀態）。**「未登錄」用紅色標出**——對外開口指向 IPAM 沒有的主機，本身就是要查的紅旗。異常偵測的「對外曝險」是抓問題，這一頁是盤點：資安稽核第一個要的東西。目的為別名／any／網段的規則**不展開猜測**（稽核拿去簽名的清單不能有猜的成分），頁面上明講涵蓋範圍。
...
### 新增
- **異常偵測新類別「防火牆規則腐化」。** 規則異動偵測盯「變更」，這一類盯「已經不對勁的現狀」：①**懸空埠轉發**——防火牆同步來的、生效中的 port forward，目標位址不在 IPAM（多半已回收，流量正轉向不明主機）；②**any → any 放行**（等於該介面沒有防火牆）；③**WAN 介面對任意來源開放管理埠**（SSH／Telnet／RDP／VNC／IPMI）。全部是確定性檢查，而且刻意保守——手動建立的 NAT 不算懸空（沒連 IP 是常態）、停用中的規則不報、LAN 開 SSH 給 any 是日常不報：這一頁的敵人是誤報，狼來了幾次就沒有人看。any-any 與管理埠檢查先做 pfSense（資料形狀最穩定），OPNsense／FortiGate 逐家驗證後再納入。
