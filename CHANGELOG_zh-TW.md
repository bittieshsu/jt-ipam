...
### 新增
- **「對外攻擊面」盤點頁**（防火牆規則異動旁；admin 與具全域讀取權的唯讀帳號——稽核員正是這頁的受眾）。彙整「從外面可達的 IP:port」：生效中的 NAT 埠轉發＋WAN 介面上目的為單一 IP 的放行規則，每一項配 IPAM 身分（主機名稱、單位／網段、有無 Wazuh 代理、實際狀態）。**「未登錄」用紅色標出**——對外開口指向 IPAM 沒有的主機，本身就是要查的紅旗。異常偵測的「對外曝險」是抓問題，這一頁是盤點：資安稽核第一個要的東西。目的為別名／any／網段的規則**不展開猜測**（稽核拿去簽名的清單不能有猜的成分），頁面上明講涵蓋範圍。
